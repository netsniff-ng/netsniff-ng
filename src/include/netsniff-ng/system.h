...
static inline int set_cpu_affinity(const char *str);
static inline char *get_cpu_affinity(char *cpu_string, size_t len);
static int set_proc_prio(int prio);
static int set_sched_status(int policy, int priority);
static inline void check_for_root(void);
static int undaemonize(const char *pidfile);
static int daemonize(const char *pidfile, const char *logfile, 
