...
static inline void destroy_virt_ring(int sock, ring_buff_t *rb);
static inline void create_virt_ring(int sock, ring_buff_t *rb);
static inline void mmap_virt_ring(int sock, ring_buff_t *rb);
static inline void bind_dev_to_ring(int sock, int ifindex, ring_buff_t *rb);
static inline void put_dev_into_promisc_mode(int sock, int ifindex);
static inline void inject_kernel_bpf(int sock, struct sock_filter *bpf, int len);
static inline int ethdev_to_ifindex(int sock, char *dev);
...
static inline int alloc_pf_sock(void);
static inline void parse_rules(char *rulefile, struct sock_filter **bpf, int *len);
...
__retry_sso:
        ...
        dbg("%.2f MB allocated for rx ring: %d blocks, %d frames, "
            "%d frames per block, framesize: %d bytes, blocksize: %d bytes \n", 
                ...
                rb->layout.tp_frame_nr,  
                rb->layout.tp_block_size / rb->layout.tp_frame_size, 
                rb->layout.tp_frame_size, 
                rb->layout.tp_block_size);
}
