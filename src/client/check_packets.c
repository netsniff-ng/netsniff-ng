...
void help()
{
	...
	printf
	    ("License GPLv2: GNU GPL version 2 <http://www.gnu.org/licenses/old-licenses/gpl-2.0.txt>\n");
	printf
...
int main(int argc, char **argv)
{
	...
	if (nagios_v) {
		printf
		    ("OK - per second: %llu frames - %llu KB, per minute: %llu frames - %llu KB\n",
		     rbs.s_per_sec.frames, DIV_KBYTES(rbs.s_per_sec.bytes),
		     rbs.s_per_min.frames, DIV_KBYTES(rbs.s_per_min.bytes));
	...
	d_day = DIV_S2DAYS(diff);
	diff = MOD_DAYS2S(diff);
	d_h = DIV_S2HOURS(diff);
	diff = MOD_HOURS2S(diff);
	d_min = DIV_S2MINUT(diff);
	diff = MOD_MINUT2S(diff);
	d_sec = diff;
	...
	printf
	    ("-----------+--------------------------+--------------------------+--------------------------\n");
	printf
	    ("           |  per sec                 |  per min                 |  total                   \n");
	printf
	    ("-----------+--------------------------+--------------------------+--------------------------\n");
	printf("  frames   | %24llu | %24llu | %24llu \n", rbs.s_per_sec.frames,
	       rbs.s_per_min.frames, rbs.total.frames);
	printf
	    ("-----------+--------------------------+--------------------------+--------------------------\n");
	printf("  in B     | %24llu | %24llu | %24llu \n", rbs.s_per_sec.bytes,
	       rbs.s_per_min.bytes, rbs.total.bytes);
	printf("  in KB    | %24llu | %24llu | %24llu \n",
	       DIV_KBYTES(rbs.s_per_sec.bytes), DIV_KBYTES(rbs.s_per_min.bytes),
	       DIV_KBYTES(rbs.total.bytes));
	printf("  in MB    | %24llu | %24llu | %24llu \n",
	       DIV_MBYTES(rbs.s_per_sec.bytes), DIV_MBYTES(rbs.s_per_min.bytes),
	       DIV_MBYTES(rbs.total.bytes));
	printf("  in GB    | %24llu | %24llu | %24llu \n",
	       DIV_GBYTES(rbs.s_per_sec.bytes), DIV_GBYTES(rbs.s_per_min.bytes),
	       DIV_GBYTES(rbs.total.bytes));
	printf
	    ("-----------+--------------------------+--------------------------+--------------------------\n");
