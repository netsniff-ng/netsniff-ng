...
extern int get_serialization_len(const gcry_mpi_t x, enum disp_format df);
extern void serialize_mpi(char *outbuf, int outlen, enum disp_format df,
			  const gcry_mpi_t x);
extern int deserialize_mpi(gcry_mpi_t * x, enum disp_format ds,
			   const char *buf, int inlen);
