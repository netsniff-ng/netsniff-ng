...
{
	size_t hdr_payload_len;
	struct auth_hdr *auth_ops;

	auth_ops = (struct auth_hdr *) pkt_pull(pkt, sizeof(*auth_ops));
	hdr_payload_len = (auth_ops->h_payload_len * 4) + 8;
	if (auth_ops == NULL || hdr_payload_len > pkt_len(pkt))
		return;
	...
	tprintf("ICV 0x");
	for (size_t i = sizeof(struct auth_hdr); i < hdr_payload_len; i++)
...
static inline void auth_hdr_less(struct pkt_buff *pkt)
{
	...
	pkt_set_proto(pkt, &eth_lay3, hdr_payload_len);
}
